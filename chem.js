...
function chemEq2Matrix (chemStr) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    for (let n = 0; n < leftLen + rightLen; n++) {
      let isElm = eqArr[n].match(new RegExp(chemElm + '[0-9]*(?=[A-Z]+|[0-9]+|$)', 'g')) || [] // 同一个式子里面有多个 同样的元素
      if (isElm.length > 0) {
  ...
  if (matrix[0].length <= matrix.length) {// 方程组大于未知数 移除有第一个主元的一行
  ...
  return { matrix: matrix, leftEqArr: leftEqArr, rightEqArr: rightEqArr, leftLen: leftLen, rightLen: rightLen, chemElmObj: chemElmObj }
}

function balanceEq (chemStr) {// render formula
    ...
    if (rst.every(x => {
      ...
      for (let i = 0; i < (m.leftLen + m.rightLen); i++) {
        if (i < m.leftLen) {
          ...
          m.rightEqArrColor[i - m.leftLen] = ($.c.y(rst[i].round(0)) + m.rightEqArr[i - m.leftLen])
          m.rightEqArr[i - m.leftLen] = (rst[i].round(0) + m.rightEqArr[i - m.leftLen])
        }
      }
      let outChemColorStr = m.leftEqArrColor.join(' + ') + '=' + m.rightEqArrColor.join(' + ')
      let outChemStr = m.leftEqArr.join(' + ') + '=' + m.rightEqArr.join(' + ')
      return { oriChem: chemStr, outChem: outChemStr, outChemCol: outChemColorStr }
    }
  }
